...
public class UsrDAOTest {
    ...
    @Test
    public void itShouldBePossibleToUpdateAUsr() throws DuplicateKeyException, KeyNotFoundException {
        Usr mum = Usr.builder().username("mum" + System.currentTimeMillis()).first_name("nicole").last_name("kamdem").build();
        Usr mumCreated = usrDao.create(mum);
        Usr mumModified = mum.toBuilder().first_name("Honorine").last_name("Kegne").build();
        usrDao.update(mumModified);
        Usr mumModifiedInDB = usrDao.find(mum.getUsername());
        assertEquals(mumModified, mumModifiedInDB);
        assertNotEquals(mumCreated,mumModifiedInDB);
